...
function render(){
 ...
 $('summary').textContent=state?`${state.students} students · ${state.groupCount} groups · no more than 5 per group`:'Up to five students in each presentation group.';
 ...
 for(let i=0;i<(state?.groupCount??0);i++){
  ...
  else{const dots=document.createElement('div');dots.className='seed-spaces';dots.setAttribute('aria-hidden','true');for(let j=0;j<5;j++)dots.append(document.createElement('span'));const p=document.createElement('p');p.textContent='Your team is taking root.';body.append(dots,p);}card.append(body);$('groups').append(card);
 }
}
